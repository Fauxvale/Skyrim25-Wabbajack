...
The main issue regarding LOOT in most Bethesda games is that its masterlists are often outdated or incomplete, leading to broken load orders that will cause mods to misbehave. Oblivion is an exception where LOOT tends to work more reliably. 
  ...
  Skyrim25's load order has already been hand-sorted to account for the conflicts between included mods, and LOOT has no way of knowing that. If you add, remove, or reorder anything, **sort it manually** rather than letting LOOT "optimize" the list for you. I know telling you to stop using LOOT if you're used to it probably sounds _super overwhelming_, but I promise it's not actually that hard. Most mods will have no real conflicts to worry about, and thus load order for those mods is irrelevant and you can put them wherever. For mods that do have notable conflicts, just load the mod that you want the changes from to win later in the load order, and/or make a patch if necessary.
